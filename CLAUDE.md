...
**Rate limiting**: `core/rate_limiter.py` wires slowapi into `main.py`, keyed by the account's email decoded straight from the JWT `sub` claim (no DB lookup) when a valid bearer token is present, else client IP (signup/login/forgot-password). Default `60/minute`; storage backend via `RATE_LIMIT_STORAGE_URI` in `core/config.py`.

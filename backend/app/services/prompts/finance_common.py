"""
Shared boilerplate for Finance interview prompts.

Extracted from finance_prompt.py so it can be reused by the
round-based builders in finance_rounds.py without duplicating the
persona intro, optional-coding structure, and style-guideline text
a second time (it's already duplicated once in finance_prompt.py,
between its resume/no-resume branches).

finance_prompt.py itself is left untouched - nothing here changes
its behavior.
"""
...
def build_role_header(role: str, difficulty: str, resume_text: str | None = None) -> str:
    ...
    return f"""You are an expert Finance interviewer hiring for leading organizations such as Goldman Sachs, JPMorgan Chase, Morgan Stanley, BlackRock, KPMG, Deloitte, EY and PwC.

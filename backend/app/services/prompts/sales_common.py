"""
Shared boilerplate for Sales interview prompts.

Extracted from sales_prompt.py so it can be reused by the round-based
builders in sales_rounds.py without duplicating the persona intro,
optional-coding structure, and style-guideline text a second time
(it's already duplicated once in sales_prompt.py, between its
resume/no-resume branches).

sales_prompt.py itself is left untouched - nothing here changes its
behavior.
"""
...
def build_role_header(role: str, difficulty: str, resume_text: str | None = None) -> str:
    """
    The "You are an expert Sales interviewer..." persona intro plus
    ...
    return f"""You are an expert Sales interviewer hiring for leading organizations such as Salesforce, Oracle, HubSpot, Microsoft, Amazon, Adobe, SAP and Cisco.
